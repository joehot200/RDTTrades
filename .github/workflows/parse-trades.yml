    ...
    steps:
      ...
      - name: Commit & push (if changed)
        ...
        run: |
          ...
          COMMIT_MSG=$(node -e "const fs=require('fs'); const p='trades/_meta.json'; if (fs.existsSync(p)) { const j=JSON.parse(fs.readFileSync(p,'utf8')); process.stdout.write(String(j.latest_commit_message || 'build: parse inbox trades').slice(0,120)); } else { process.stdout.write('build: parse inbox trades'); }")
